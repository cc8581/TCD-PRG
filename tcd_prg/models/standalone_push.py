...
class StandalonePushModel(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.push = PushHead(config.feature_dim, config.num_direction_bins,
    ...
    def forward(self, batch: Mapping[str, Any], *, forward_mode: str = "push") -> dict[str, Any]:
        ...
        sensor = self._sensor(batch); condition = batch.get("push_condition")
        ...
        return {"push_condition": condition, "sensor": sensor,
            "task": {"task_category_id": condition.task_category_id, "task_region_id": condition.task_region_id},
            "encoded": None, "instance": None, "region": None, "task_grasp": None,
            "global_grasp": None, "push": push}
